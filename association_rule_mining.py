...
for index, row in df.iterrows():
    ...
    in_transaction.append(list(set(itemset).intersection(row)))
    #print("in transaction: " + str(in_transaction))
    not_in_transaction.append(list(set(itemset).difference(row)))
    #print("not in transaction: " + str(not_in_transaction))


    labels = {}
    for i in range(len(in_transaction[0])):
        labels[in_transaction[0][i]] = 1

    for i in range(len(not_in_transaction[0])):
        labels[not_in_transaction[0][i]] = 0

    in_transaction = []
    not_in_transaction = []
    encoded_vals.append(labels)
    #print("encoded vals: " + str(encoded_vals))
...
for index, rule in rules.iterrows():
    ...
    print(', '.join(rule['antecedents']) + " -> " + ', '.join(rule['consequents']))
